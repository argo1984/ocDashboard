<form id="ocDashboardSettings" class="section">
	...
	<?php 
		foreach ($_['widgets'] as $widget) {
			...
			<label class="widgetName" for="ocDashboard_<?php p($id); ?>" >&nbsp;&nbsp;<?php p($l->t($name)); ?></label>
                ...
                if(isset($widget['conf']) && !empty($widget['conf'])) { ?>
                    <span class="widgetSettingsTriangle icon-triangle-s <?php p($id); ?>" data-expanded="0" data-id="<?php p($id); ?>">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</span>
                <?php
                } ?>

			<br />

            <div class="widgetSettings <?php p($id); ?>">
                <br />
                <?php
                if(isset($widget['conf']) && !empty($widget['conf'])) {
		...
		print_unescaped($l->t('<p style="padding-top: 20px;">There are some background colors for the widgets.<ul><li class="colorRed">red: Error</li><li class="colorOrange">orange: missconfiguration</li><li class="colorGreen">green: there is something new</li><li></li></ul></p>'));
	?>
